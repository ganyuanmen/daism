...
export default function SwapWindow({workIndex,show,setShow,selectToken}) { 
    ...
    return  <Modal show={show} scrollable={true} className="daism-title" 
    // onShow={e=>{inputRef.current.value='';if(tokenList.length!==tokenFilter.length) dispatch(setTokenFilter(tokenList));}}  
            ...
            <Modal.Header style={{padding:0,margin:0}} >
                <div style={{width:'100%'}} >
                    ...
                    <div style={{backgroundColor:'white',width:'100%',padding:'10px'}} >
                        ...
                        {workIndex===-1 &&  <Button className={iaddStyle.iadd_btn} variant="outline-secondary"  onClick={()=>clickSelect(utokenObj)} >
