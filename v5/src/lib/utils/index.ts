...
export  function findFirstURI(code:string) {
  ...
  const regex = /(?<!<(?:img|a|image|use|feimage|svg)[^>]*(?:src|href|xlink:href|xmlns)=["'])(?<!url\(["']?)(https?:\/\/[^\s"'<>)]+)(?!["']?\))/i;
...
export function getClientIpPage(headers: ReadonlyHeaders): string | undefined {
  // 从 headers 中获取 x-forwarded-for
