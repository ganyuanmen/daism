...
export default function EnKiRigester({ setRegister }: EnKiRigesterProps) {
  ...
  const register = async () => {
    ...
    if (!actorName || actorName.length > 12 || /[./\\]/.test(actorName)) {
    ...
    const re = await fetchJson<UserRegister>(`/api/getData?account=0x${actorName}@${domain}&did=${user.account}`,{headers:{'x-method':'getSelfAccount'}});
    if(!re) { 
       showClipError('Data processing failed. Please try again later.');
       closeTip();
      return;
    }
    ...
    daismObj?.Domain.recordInfo(actorName, domain).then(
      () => {
        ...
        fetch('/api/postwithsession',{
            method:'POST',
            headers:{'x-method':'insertRegisterLog'},
            body:JSON.stringify({did:user.account})
        });
      ...
      (err) => {
        ...
        // showClipError(tc('errorText') + (err.message ? err.message : err));
          if(err.message && err.message.toString().includes(' rejected ')){
                    showClipError(tc("rejectText"));
                    return;
                }
                if(err.message && err.message.toString().includes('insufficient ')){
                    showClipError(tc("rejectText"));
                    return;
                }


                showClipError(tc('droppedText'));
