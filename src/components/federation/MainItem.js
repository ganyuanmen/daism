    ...
    return (
        ...
        <Card className="daism-title" >
            <Card.Header className="d-flex align-items-center justify-content-between m-0 p-1"  >
                ...
                <div>
                   {!isVist &&  <Link  className="btn btn-primary"  href={`/${locale}/info/${path}/new/[id]`} as={`/${locale}/info/${path}/new/${daoid}`}  ><AddSvg size={20} />  {cardData[path].newTitle}</Link>  }
                </div>
                <div>
                    <Link  href={`/${locale}/info/${path}/list/[id]`} as={`/${locale}/info/${path}/list/${daoid}`} style={aStyle} > <FindSvg /> {t('serachText')}... </Link>
                </div>
            </Card.Header>
            <Card.Body style={{minHeight:'220px'}} >
                ...
                <div className="d-flex justify-content-start align-items-start flex-wrap mb-3 g-3" >
                    {data.map((obj,idx)=>(
                        <Link className="daism-a"  style={aStyle} key={path+idx} href={`/${locale}/info/events/message/[id]`}  as={`/${locale}/info/events/message/${obj.id}`}  >
                ...
                data.map((obj,idx)=>(
                    <Link key={path+idx}  style={aStyle}  href={`/${locale}/info/${path}/message/[id]` }  as={`/${locale}/info/${path}/message/${obj.id}`} >
