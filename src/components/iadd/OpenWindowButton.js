...
export function OpenWindowButton({user,setOutput,workIndex,token,setToken,setBalance,comulate,utoken,iadd,setTokenPrice,...props}){
    ...
    const selectToken = async (obj) => {
        ...
        if (obj.token_id === -2) {logoImage = '/eth.png';setTokenPrice('');}
            ...
            if(workIndex===-2 && window.daismDaoapi ){ checkApproveUtoken() }
            logoImage = '/uto.svg';setTokenPrice('');
        }
        else { //token
            if(workIndex===-2 && window.daismDaoapi){ checkApproveToken() }
            ...
            if (inobj.token_id === -2) { //上窗口eth
               ...
               utoken.getOutputAmount(getEther(1)).then(ethToutokenPrice=>{
                if (outobj.token_id === -1) { //eth to utoken
                    props.statusRef.current.setStatus({ratio:`1 ETH = ${fromEther(ethToutokenPrice[0])} UTOKEN` })
                } else {  // eth to token
                    if (outobj.token_id) { 
                ...
                if (outobj.token_id) {
                    comulate.unitTokenToSCToken(getEther(1), outobj.token_id).then(e => {
                ...
                if (outobj.token_id === -1) {  //token to utoken
                    comulate.SCTokenToUnitToken(getEther(1), inobj.token_id).then(e => {
                        props.statusRef.current.setStatus({ratio:`1 ${inobj.btext} = ${fromEther(e)} UTOKEN`})
                    })
                } else {  //token to token
