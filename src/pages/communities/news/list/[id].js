...
export default function ListsPage({daoDataServer}) {
  ...
  return (
    <PageLayout>
           ...
           <Breadcrumb menu={[{url:`/communities/visit/${daoData.dao_id}`,title:daoData.dao_name}]} currentPage='news' ></Breadcrumb>
           {!isVist && <Link className="btn btn-primary" href={`/communities/news/new/[id]`} as={`/communities/news/new/${daoData.dao_id}`} > <AddSvg size={20} />{t('createNews')} </Link>}
