...
export default function ListsPage({daoDataServer}) {
  ...
  return (
    <PageLayout>
            ...
            {!isVist && <Link className="btn btn-primary" href={`/communities/events/new/[id]`} as={`/communities/events/new/${daoData.dao_id}`} > <AddSvg size={20} />{t('createEventsText')} </Link> }
...
function ListMain({rows,t})
{
  return <>
         <div className="d-flex justify-content-start align-items-start flex-wrap mb-3 g-3" >
            {rows.map((obj,idx)=>(
              <Link key={idx} className='daism-a' href={`/communities/events/message/[id]`} as={`/communities/events/message/${obj.id}`} >
