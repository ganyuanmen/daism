...
export default function ListsPage({daoDataServer}) {
  ...
  const pageData=useInfoList({currentPageNum,daoid:daoData.dao_id,pages:10,method:'discussionPageData'})
  ...
  return (
    <PageLayout>
            ...
            <Breadcrumb menu={[{url:`/communities/visit/${daoData.dao_id}`,title:daoData.dao_name}]} currentPage='discussions' ></Breadcrumb>
           {!isVist && <Link className="btn btn-primary" href={`/communities/discussions/new/[id]`} as={`/communities/discussions/new/${daoData.dao_id}`} > <AddSvg size={20} />{t('addDiscussionText')} </Link>}
