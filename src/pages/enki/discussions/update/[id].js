...
function NewMain({discussionData,tc,t}) {
  ...
  async function submit()
  {
    ...
    if(res.status===200)  router.push(`/enki/discussions/message/${discussionData.id}`, { scroll: false })
    else showClipError(`${tc('dataHandleErrorText')}!${res.statusText}\n ${res.data.errMsg}`)
    closeTip()
  }

  const menu=[
      {url:`/enki/visit/${discussionData.dao_id}`,title:discussionData.dao_name},
      {url:`/enki/discussions/list/${discussionData.dao_id}`,title:'discussions'}
  ]
