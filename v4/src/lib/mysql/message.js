...
export async function fromAccount({actor_account,user_account}){
	...
	if(actor_account.includes(process.env.LOCAL_DOMAIN)){ //本地帐号
		...
		let sql='SELECT actor_name `name`, actor_inbox inbox,domain,actor_account account,actor_url url,avatar,pubkey,manager FROM v_account where actor_account=? || actor_url=?';
