 ...
 * @currentObj 嗯文或回复的对象
 */
export default function TimesItem({currentObj}) {  
    ...
    const geneTimes=()=>{
        ...
        if(diffInMinutes <60) return `${diffInMinutes>>0} ${t('minute')}`;
        else if(diffInHours<24) return `${diffInHours>>0} ${t('hour')}`;
        else if(diffInDays<7) return `${diffInDays>>0} ${t('day')}`;
        else {
    ...
    return (
        <span> <TimeSvg size={24} /> {geneTimes()} </span>
    );
}
